...
- **`design/*.md`**. Describes intended behavior — architecture, cross-cutting concerns, design rationale. Update after a PR is code-complete and tested, but only if a design decision was made. If a file is being deleted or downsized, update inbound references.
...
- **Embedded crates: `workgraph/` and `cellgraph/`**. Each embedded library carries its own doc tree, mirroring the partition above: `<crate>/README.md` introduces the crate and indexes the tree ([`workgraph/README.md`](../../../workgraph/README.md), [`cellgraph/README.md`](../../../cellgraph/README.md)), `<crate>/design/*.md` describes that library's design, and `<crate>/roadmap/` holds its open work with its own derived `## Next items` slice ([workgraph's](../../../workgraph/roadmap/README.md), [cellgraph's](../../../cellgraph/roadmap/README.md)) — each crate is one project, so that README is both the tree index and the project README. **Which tree a doc belongs to:** a doc describing a library *as a standalone library* goes in that crate's tree; a doc describing the *boundary* — what is library, what is koan — stays koan-side, which is why [`design/scheduler-library.md`](../../../design/scheduler-library.md), `design/per-node-memory.md`, and `design/witness-hosting.md` do. Roadmap items across all three trees form **one dependency graph** — a cross-tree `Requires:`/`Unblocks:` edge is gated for symmetry like any other — but no tree's `## Next items` list reaches across the split.
