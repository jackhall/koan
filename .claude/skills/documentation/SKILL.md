...
- **Module `README.md` design docs.** A module's design doc is the `README.md` in its source directory (`src/parse/README.md`, `cellgraph/src/graph/README.md`): intended behavior — architecture, cross-cutting concerns, design rationale — for that module, linked from its top-of-file comment. Update after a PR is code-complete and tested, but only if a design decision was made. A cross-module invariant lives in the README of the module that enforces it. `doclinks` link-checks `src/**/*.md` across koan and the embedded crates.
- **`old_design/*.md`**. **Retired.** The topical design tree of the runtime behind `pending_rewrite`, plus the docs for kept modules until [design-docs-in-modules.md](../../../roadmap/rewrite/design-docs-in-modules.md) moves each into its module. Read it as requirements and as the source of a migration; never add a doc there. `cellgraph/design/` is not retired but migrates under the same item. If a file is being deleted or downsized, update inbound references.
...
- **Embedded crates: `workgraph/` and `cellgraph/`**. Each embedded library carries its own doc tree, mirroring the partition above: `<crate>/README.md` introduces the crate and indexes the tree ([`workgraph/README.md`](../../../workgraph/README.md), [`cellgraph/README.md`](../../../cellgraph/README.md)), `<crate>/design/*.md` describes that library's design (migrating into module READMEs per the bullet above), and `<crate>/roadmap/` holds its open work with its own derived `## Next items` slice ([cellgraph's](../../../cellgraph/roadmap/README.md); workgraph's sits retired at [workgraph/old_roadmap/](../../../workgraph/old_roadmap/README.md)) — each crate is one project, so that README is both the tree index and the project README. **Which tree a doc belongs to:** a doc describing a library *as a standalone library* goes in that crate's tree; a doc describing the *boundary* — what is library, what is koan — stays koan-side, which is why [`design/scheduler-library.md`](../../../old_design/scheduler-library.md), `design/per-node-memory.md`, and `design/witness-hosting.md` do. Roadmap items across all three trees form **one dependency graph** — a cross-tree `Requires:`/`Unblocks:` edge is gated for symmetry like any other — but no tree's `## Next items` list reaches across the split.
...
- **orphaned design/ + roadmap/ docs.** Every `design/*.md`, `old_design/*.md` and `roadmap/*.md` file that no other doc, comment, or source file links to. An orphan is usually either a new doc that needs an entry in `README.md` / `roadmap/README.md`, or a stale doc that should be deleted.
