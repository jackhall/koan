...
- **`workgraph/`**. The embedded scheduler/region-memory library carries its own doc tree, mirroring the partition above: [`workgraph/README.md`](../../../workgraph/README.md) introduces the crate and indexes the tree, `workgraph/design/*.md` describes the library's design, and [`workgraph/roadmap/`](../../../workgraph/roadmap/README.md) holds its open work with its own derived `## Next items` slice (the crate is one project, so that README is both the tree index and the project README). **Which tree a doc belongs to:** a doc describing the library *as a standalone library* goes in `workgraph/`; a doc describing the *boundary* — what is library, what is koan — stays koan-side, which is why [`design/scheduler-library.md`](../../../design/scheduler-library.md), `design/per-node-memory.md`, and `design/witness-hosting.md` do. Roadmap items across both trees form **one dependency graph** — a cross-tree `Requires:`/`Unblocks:` edge is gated for symmetry like any other — but neither tree's `## Next items` list reaches across the split.
