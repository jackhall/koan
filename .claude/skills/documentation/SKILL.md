...
- **`roadmap/README.md`**. A curated index of the `roadmap/` work items. The "What's shipped so far" prose paragraph is the running record of completed work; the bulleted "Open items" sections list active items. The "Next items" section is **not** hand-maintained — it is derived from the dependency graph (every item with no still-open prerequisite) and regenerated by `doclinks sync-next`; `doclinks check` gates it.
...
2. Add a bullet to `roadmap/README.md` under the right "Open items" subsection. Leave `## Next items` alone — it is derived (step 4 regenerates it).
3. If the new item is unblocked by something, add a back-edge `Unblocks: <new-item>` in that prerequisite item's Dependencies.
4. Run `python3 tools/doclinks.py sync-next` so the new item lands in (or stays out of) `## Next items` per its `Requires:` edges, then `python3 tools/doclinks.py check` to confirm bidirectional dependency symmetry, that the Next-items list is in sync, and that the new file is wired in (the orphans section will flag an unreferenced doc).
...
When a roadmap item ships, this command deletes the file *and* prunes its inbound bullets in a single pass: every `**Requires:**` / `**Unblocks:**` bullet in other `roadmap/*.md` items that links to it, plus any bullet under `## Open items` in `roadmap/README.md`. Continuation lines belonging to a removed bullet are dropped too. It then regenerates the derived `## Next items` list (via `sync-next`) from the now-smaller graph, so any dependent the delete just unblocked is added automatically. Finally it runs `check` and propagates its exit code, so any remaining stale references (design-doc "Open work" sections, source-file comments, prose mentions inside Dependencies sections) surface immediately. The tool does **not** touch the `What's shipped so far` paragraph or those remaining-reference sites — those need a judgment call. Use `--dry-run` to preview.
