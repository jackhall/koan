...
6. If the plan seems reasonable or can't be decomposed, bring the roadmap item up-to-date and continue to preflight.
...
- Respect the plan. Use `ToDoWrite` to keep yourself on track. Commit after each step to keep the suite green (via the pre-commit hook), and fix regressions before moving on.
