...
**Whitelisting is not the default response to a removed `unsafe`.** When you delete an `unsafe` site — or relocate its backing op to another file (e.g. fold a layout-invariance `unsafe impl` into the `reattachable!` macro, whose `unsafe` then lives once in `witnessed.rs`) — a group's anchor file can go `unsafe`-free. Do **not** reflexively whitelist it to silence the check. Ask of each test under that group: *can it catch a memory error no other slate test catches?* If yes — keep the test and whitelist the anchor, citing the distinct UB shape and where the real `unsafe` now lives (e.g. a fat-pointer `Box<dyn>` reattach that thin-carrier tests miss). If no — the test is redundant; **delete it** and remove its group, rather than whitelist. Whitelisting earns its keep only for tests that still pin a unique failure mode; a test that merely mirrors a now-deleted `unsafe` with no unique coverage is dead weight.
