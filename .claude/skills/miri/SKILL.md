...
- Do not check whether Miri is installed.
- Do not run Miri in the foreground for slate-sized work.
- Do not poll `BashOutput` or use sleep loops while waiting.
- Do not interleave Miri runs with `cargo test` if you can avoid it.
- Do not guess at the audit slate's test names — read them from `roadmap/post-stage-1-audit-redo.md`.
