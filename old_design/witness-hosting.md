...
([reach.md § The carrier states](../workgraph/old_design/reach.md#the-carrier-states)).
...
- An **undeclared return** ends the same way: the value stays in its producer
  ...
  ([reach.md § Retention model](../workgraph/old_design/reach.md#retention-model)).
...
[region evacuation](../roadmap/old_untyped_arena/region-evacuation.md) collapses the
...
- **Raw AST** takes `RegionBrand::alloc_expression`, which admits a
  `ProgramExpression` and nothing else — the marker
  minted only through a `ProgramBrand` door. A
...
`SealedExtern` re-anchored to the same `'b`, under a [`Witness`](../workgraph/old_design/witnessed-memory.md)
