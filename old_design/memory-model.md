...
on the value itself: a producer slot's `FrameCoverage` pins it while the value rides
...
scope evicted off the lifetime-free node) rides the substrate's externally-witnessed
...
[witnessed-memory.md § Storage and access](../workgraph/old_design/witnessed-memory.md#storage-and-access-seal-open-transfer_into))
...
([dag-scheduler.md § Delivery at finalize](../workgraph/old_design/dag-scheduler.md#delivery-at-finalize)),
...
([witnessed-memory.md](../workgraph/old_design/witnessed-memory.md)). The
...
reach on their [delivered carrier](../workgraph/old_design/witnessed-memory.md#storage-and-access-seal-open-transfer_into): a
...
([reach.md § Composition](../workgraph/old_design/reach.md#composition-minting-a-description-and-retaining-its-pins)).
...
are *not* here — they belong to [`execute::RunFrame`](../src/machine/execute/run_frame.rs), which
owns the frame that adopts the run-root scope alongside them. The shell/storage split lets an escaping value pin only the storage (its region), so the
...
- **Field declaration order encodes drop order.** On `FrameStorage`, `region` is declared
  ...
  releasing the parent storage Rc. The shell needs no such rule: its two halves arrive coupled in
  one envelope, and dropping a sealed carrier never dereferences the child pointer.
...
it wrapped as a [`StepCarried`](../src/machine/execute/step.rs) branded at the step's `'step`
...
- **folded** (`FoldingBrand::alloc_object_folded` / `alloc_cell_folded` / `alloc_substrate_folded` /
  ...
  (`KObject<'b>` on `FoldingBrand<'b>`), and inside a fold combinator's `for<'b>` closure the only inhabitants of that
...
([reach.md § The carrier states](../workgraph/old_design/reach.md#the-carrier-states)); what it holds is
...
([`Scope::open_frame`](../src/machine/core/scope.rs)) is the one whose operand — the lexical
...
[`StepAllocator::alloc_carried_with`](../src/machine/execute/step.rs),
...
edges](../workgraph/old_design/dag-scheduler.md#pushnotify-dependency-edges)) keeps its slot-table
state in a
...
- The audit slate runs cycle-free across every unsafe site in the runtime
  ...
  slate list lives in [observe/miri_slate_pending_rewrite.md](../observe/miri_slate_pending_rewrite.md).

## Open work

- [Tightness-audit coverage](../roadmap/old_compile_safety/tightness-audit-coverage.md)
