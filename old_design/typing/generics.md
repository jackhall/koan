...
free-type-parameter form: a type parameter is always an argument position, and so
always has a binder.

## Shape

A generic function is a definition taking one or more `:Type` parameters and
returning a module that holds the function specialized to those types:

```
LET make_head = FN EXPR (MAKEHEAD Ty :Type) -> Module = (
  MODULE built = ((EXPR (HEAD xs :(LIST OF Ty)) -> Ty = ...))
)
```

Inside the body `Ty` is a bound parameter, so the inner definition is ordinary:
`Ty` resolves through the outer definition's per-call scope, the same path body-position
...
no syntax beyond the definition binder and an ordinary call.

## Why functors

- **One surface.** A type parameter is always an argument position — one spelling,
...
- **Operation-bearing** — `sort` consults an ordering that is not contained in
  ...
  ([equivalence-checking.md](../../roadmap/old_predicate_typing/equivalence-checking.md)):
...
— are expressed by lifting the type to a parameter of the outer definition
(`EXPR (MAKEIT Ty :Type) -> Module = (MODULE built = ((EXPR (MAKE elt :Ty) -> ...)))`),
