...
[workgraph/old_design/reach.md](../workgraph/old_design/reach.md) owns the reach
...
region.rs holds only that profile
...
`KExpression` handles (the `KObject::KExpression`
...
- **The value channel borrows program storage.** Every `KObject::KExpression` cell
  ...
  `ProgramStorage` the parse door bumped it
  ...
  `ProgramExpression`, minted only through a
  `ProgramBrand` door, so a node the runtime
...
only a `ProgramExpression` reaches it, so the
...
`RegionBrand::seal_resident` surface, reached by the
...
a `FrameCoverage` built at step start over the
...
- **The run's program brand** enters by the brand bound rather than by channel.
  ...
  `ProgramBrand<'run>` the runtime holds be
...
- **Frame-side reads** fold onto `open` the same way: a frame's own child scope
  ...
  `CallFrame::with_resident` — the `&mut self` submit /
