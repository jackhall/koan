...
/// 100 tail-recursive steps, 176.0 allocations each — exactly linear, measured flat at
/// 10/50/100/200. Measured 2026-08-19 at 20 778; the bound is that plus 41, less than the 100
...
/// A 128-operand `+` chain, so 127 dispatches at ≈49 allocations each — mildly superlinear,
/// with marginal cost rising 47.9 → 50.3 across the 16→32 … 128→256 operand doublings (below
/// 16 operands it is flat near 47.4). Measured 2026-08-19 at
/// 9 108; the bound is that plus 41, under the 127 a single new per-dispatch allocation
/// would add. Same headroom rule as the loop.
#[test]
fn the_operator_chain_shape_stays_within_its_dispatch_churn_bound() {
    const BOUND: u64 = 9_149;
    let delta = allocations_for(
