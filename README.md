...
└── machine/
    ...
    └── execute/
        ...
        ├── runtime.rs     KoanRuntime — owns the Scheduler, the sole &mut holder: the execute loop, apply_outcome (sole graph writer), the AST-aware submission wrappers, submit_dispatch, literal lowering; plus run_action (lowers a builtin Action to an Outcome, pure); interpret/ (the program entry points + run_program) submodule
