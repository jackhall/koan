...
//! plus the bump-allocation seam the registry and every scratch buffer are built over and the
//! component walk from `memory` — with, in its own suite, the lib-test binary's allocation counter. The compiler cannot
...
/// writable table is built through, the component walk a recursive group condenses by, the macro that mints a static name, the lib-test allocation
