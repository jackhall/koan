...
//! `(name, type)` fields and the FN parameter list. Generic over the value, though the lattice's
//! nodes are the only residents, so `Record<KType>` is what it is instantiated at.
//!
