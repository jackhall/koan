...
pub fn join(types: &TypeRegistry, a: KType, b: KType) -> KType {
    if is_subtype_of(types, a, b) {
        return b;
    }
    if is_subtype_of(types, b, a) {
        return a;
    }
    types.union_of(&[a, b])
}
...
pub fn join_iter<I: IntoIterator<Item = KType>>(types: &TypeRegistry, iter: I) -> KType {
    iter.into_iter()
        .reduce(|a, b| join(types, a, b))
...
pub fn meet(types: &TypeRegistry, a: KType, b: KType) -> KType {
    lockstep(types, a, b, Variance::Co, &mut Meet)
}
...
impl Lockstep for Meet {
    ...
    fn enter(&mut self, types: &TypeRegistry, a: KType, b: KType, v: Variance) -> Option<KType> {
        if v == Variance::Contra {
            return Some(join(types, a, b));
        }
        if a == b {
            return Some(a);
        }
    ...
    fn leaf(&mut self, types: &TypeRegistry, a: KType, b: KType, _v: Variance) -> KType {
        ...
        let met = types.with_node(a, |na| {
            types.with_node(b, |nb| match (na, nb) {
                (TypeNode::Signature { schema: x, .. }, TypeNode::Signature { schema: y, .. }) => {
                    meet_schemas(types, x, y)
                }
                _ => None,
            })
        });
        match met {
            Some(schema) => types.signature(schema),
            None => KType::NEVER,
        }
    }
    ...
    fn structural(&mut self, _types: &TypeRegistry, paired: &[KType], arm: Arm<'_>) -> KType {
