...
use crate::machine::model::{ExpressionSignature, KObject, KType, SignatureElement};
use crate::machine::{ArgumentBundle, Body, BodyResult, CombineFinish, KError, KErrorKind, KFunction, Scope, SchedulerHandle};
use crate::machine::model::types::{DeferredReturn, Elaborator, ReturnType};
...
fn type_expr_references_any(te: &crate::machine::model::ast::TypeExpr, param_names: &[String]) -> bool {
...
fn kexpression_references_any<'a>(
    expr: &crate::machine::model::ast::KExpression<'a>,
    param_names: &[String],
) -> bool {
...
fn part_references_any<'a>(
    part: &crate::machine::model::ast::ExpressionPart<'a>,
    param_names: &[String],
) -> bool {
    match part {
...
pub fn body<'a>(
    ...
    enum ReturnTypeRaw<'a> {
        Resolved(KType),
        TypeExprCarrier(crate::machine::model::ast::TypeExpr),
        ExprCarrier(crate::machine::model::ast::KExpression<'a>),
    }
    ...
    enum ReturnTypeState<'a> {
        ...
        Pending {
            ...
            producers: Vec<crate::machine::NodeId>,
        },
        Deferred(DeferredReturn<'a>),
        ExprSubDispatched(crate::machine::NodeId),
    }
...
fn make_capture<'a>(te: crate::machine::model::ast::TypeExpr) -> ReturnTypeCapture<'a> {
    use crate::machine::model::ast::TypeParams;
    match te.params {
...
enum ParamListResult<'a> {
    ...
    Pending {
        park_producers: Vec<crate::machine::NodeId>,
        sub_dispatches: Vec<(usize, crate::machine::model::ast::KExpression<'a>)>,
    },
}
...
enum ReturnTypeCapture<'a> {
    Resolved(KType),
    Unresolved(String),
    TypeExpr(crate::machine::model::ast::TypeExpr),
...
fn finalize_fn<'a>(
    ...
    body_expr: crate::machine::model::ast::KExpression<'a>,
) -> BodyResult<'a> {
...
fn defer_via_combine<'a>(
    scope: &'a Scope<'a>,
    sched: &mut dyn SchedulerHandle<'a>,
    signature_expr: crate::machine::model::ast::KExpression<'a>,
    ...
    let mut deps: Vec<crate::machine::NodeId> = park_producers;
    ...
    let finish: CombineFinish<'a> = Box::new(move |scope, _sched, results| {
        ...
        let spliced_signature = crate::machine::model::ast::KExpression { parts: spliced_parts };
        ...
        let return_type: ReturnType<'a> = match return_type_capture {
            ...
            ReturnTypeCapture::Unresolved(name) => {
                let te = crate::machine::model::ast::TypeExpr::leaf(name.clone());
                match scope.resolve_type_expr(&te) {
