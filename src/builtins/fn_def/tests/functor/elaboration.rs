...
fn elaborator_lowers_ktype_value_binding() {
    ...
    match elaborate_type_identifier(&mut el, type_token("MyList"), test_run.registries()) {
        TypeResolution::Done(kt) => assert_eq!(kt, test_run.types().list(KType::NUMBER)),
...
fn fn_with_signature_bound_param_records_signature_bound_ktype() {
    ...
    match f.signature.elements() {
        [
            ...
            assert_eq!(name.symbol(), crate::parse::Symbol::of("er"));
