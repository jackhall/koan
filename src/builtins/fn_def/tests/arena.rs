...
/// the harness routes arm-body leading statements through the same owned-dep park: the leading
/// `PRINT` runs before the recursion continues, giving `hop` (the One arm) then `done` (the Zero
/// arm) in order.
#[test]
