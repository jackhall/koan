...
pub fn body<'a, 's>(
    ...
    let (result_set, result_index) = match sched.current_scope().resolve_type("Result") {
    ...
    let finish: CatchFinish<'a> =
        Box::new(move |_sched: &mut dyn SchedulerHandle<'a, '_>, result| {
            let (tag, payload): (&str, KObject<'a>) = match result {
                Ok(v) => ("Ok", v.deep_clone()),
                Err(e) => ("Error", e.to_tagged(_sched.current_scope().arena)),
            };
            ...
            BodyResult::value(_sched.current_scope().arena.alloc_object(tagged))
        });
