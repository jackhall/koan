...
/// `USING` on a non-module value: cache-driven strict admission inspects `n`'s
/// resolved carrier (a Number) against the `m :Module` slot and rejects upfront.
/// No other USING overload admits, so the post-walk surfaces `DispatchFailed` —
/// replacing the pre-PR-C bind-time `TypeMismatch` (which flowed from
/// tentative-admit then a bind step rejecting the Number-carrier against the
/// Module slot). PR C surface change per `scratch/plan-unified-walk-pr-c.md`.
#[test]
fn using_on_non_module_fails_dispatch() {
    ...
    assert!(
        ...
        "expected DispatchFailed (PR C strict-only surface) for USING on a Number, got {err}",
    );
}
