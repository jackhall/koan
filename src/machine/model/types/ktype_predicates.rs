...
/// they lower — reached either through a union that reduces to them (`capture_member_for`) or at a
/// bare kind slot that shape-admitted a park-exempt binder operand
/// (`WorkingExpression::park_exempt_slot`), the one token such a slot does not resolve. Claiming
/// the token is also what keeps a kind member and `TypeNameToken` out of one union, the one shape
/// over which the two would disagree. Every other slot type — a value type, a container, a
/// nominal — claims nothing.
