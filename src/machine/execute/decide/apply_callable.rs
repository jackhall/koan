...
fn extract_call_body<'step>(
    ...
    match &expr.parts[1..] {
        ...
        _ => Err(KError::new(KErrorKind::DispatchFailed {
            expr: expr.summarize(registries),
...
pub(in crate::machine::execute) fn apply_callable<'step>(
    ...
    match callable {
        ...
        ResolvedCallable::Function(f) => {
            let body = match extract_call_body(expr, ctx.registries()) {
                Ok(b) => b,
...
fn apply_function<'step>(
    ...
    match body {
        ...
        CallBody::Positional(_) => body_shape_err(expr, NAMED_ONLY, ctx.registries()),
    }
}
...
pub(in crate::machine::execute) fn install_eager_subs_track<'step>(
    ...
    let brand = ctx.current_scope().brand();
    // No pre-pick `bare_outcomes` on this lane, so no slot is parked: the empty mask says so.
    let wrap_indices = picked
        .value()
        .classify_for_pick(&expr, &[], ctx.types(), ctx.scratch());
