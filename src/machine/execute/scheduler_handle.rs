//! The `SchedulerHandle` trait and the `Combine`/`Catch` finish aliases. These live in `execute`
//! (rather than `core/kfunction`) because their finish aliases return [`Outcome`], which is capped
//! at `pub(in crate::machine::execute)`; a `pub` alias returning a crate-private `Outcome` would be
//! a private-in-public error. `execute/scheduler.rs` impls `SchedulerHandle` for `Scheduler`.
