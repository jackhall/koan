...
pub(in crate::machine::execute) fn schedule_list_literal<'run>(
    ...
    for part in items {
        ...
        let slot = classify_aggregate_part(sched, part, &mut deps, &mut park_producers, false);
        layout.push(slot);
    }
    ...
    sched.combine_here(deps, park_producers, finish)
}
...
pub(in crate::machine::execute) fn schedule_dict_literal<'run>(
    sched: &mut Scheduler<'run>,
    ...
    let mut park_producers: Vec<NodeId> = Vec::new();
    for (k, v) in pairs {
        let key_slot = classify_aggregate_part(sched, k, &mut deps, &mut park_producers, true);
        let val_slot = classify_aggregate_part(sched, v, &mut deps, &mut park_producers, true);
...
pub(in crate::machine::execute) fn schedule_record_literal<'run>(
    sched: &mut Scheduler<'run>,
    ...
    for (name, value) in fields {
        let val_slot = classify_aggregate_part(sched, value, &mut deps, &mut park_producers, true);
        names.push(name);
...
fn classify_aggregate_part<'run>(
    sched: &mut Scheduler<'run>,
    ...
    match part {
        ExpressionPart::ListLiteral(inner) => {
            let nested_id = schedule_list_literal(sched, inner);
            let pos = deps.len();
            deps.push(nested_id);
            Slot::Owned(pos)
        }
        ExpressionPart::DictLiteral(inner) => {
            let nested_id = schedule_dict_literal(sched, inner);
            let pos = deps.len();
            deps.push(nested_id);
            Slot::Owned(pos)
        }
        ExpressionPart::RecordLiteral(inner) => {
            let nested_id = schedule_record_literal(sched, inner);
            let pos = deps.len();
            deps.push(nested_id);
            Slot::Owned(pos)
        }
        ExpressionPart::Expression(boxed) => {
            let sub_id = sched.dispatch_here(*boxed);
            let pos = deps.len();
            deps.push(sub_id);
            Slot::Owned(pos)
        }
        ExpressionPart::SigiledTypeExpr(_) | ExpressionPart::RecordType(_) => {
            ...
            let wrapped = crate::machine::model::ast::KExpression::new(vec![Spanned::bare(part)]);
            let sub_id = sched.dispatch_here(wrapped);
            let pos = deps.len();
            deps.push(sub_id);
            Slot::Owned(pos)
        }
        ref p @ ExpressionPart::Identifier(_) if wrap_identifiers => {
            resolve_aggregate_bare_name(sched, p, deps, park_producers)
        }
...
fn resolve_aggregate_bare_name<'run>(
    sched: &mut Scheduler<'run>,
    part: &ExpressionPart<'run>,
    deps: &mut Vec<NodeId>,
    park_producers: &mut Vec<NodeId>,
) -> Slot<'run> {
    match resolve_name_part(sched.current_scope(), part, sched, None) {
        ...
        NameOutcome::Unbound(_) | NameOutcome::ProducerErrored(_) | NameOutcome::Cycle(_) => {
            ...
            let sub_id = sched.dispatch_here(expr);
            let pos = deps.len();
            deps.push(sub_id);
            Slot::Owned(pos)
        }
