...
use crate::machine::{FrameStorage, KFuture, KoanRegion};
...
/// an `Rc<FrameStorage>` to anchor any descendant that borrows into the dying region.
...
#[cfg(test)]
pub fn lift_ktype_for_test<'run>(t: &KType<'run>, dying_frame: &Rc<FrameStorage>) -> KType<'run> {
    lift_ktype(t, dying_frame)
}

pub(super) fn lift_kobject<'run>(v: &KObject<'run>, dying_frame: &Rc<FrameStorage>) -> KObject<'run> {
...
pub(super) fn lift_ktype<'run>(t: &KType<'run>, dying_frame: &Rc<FrameStorage>) -> KType<'run> {
    match t {
        KType::Module {
            module: m,
            frame: existing,
...
fn needs_lift<'run>(v: &KObject<'run>, dying_frame: &Rc<FrameStorage>) -> bool {
    let dying_runtime: *const KoanRegion = dying_frame.region();
...
fn lift_held<'run>(cell: &Held<'run>, dying_frame: &Rc<FrameStorage>) -> Held<'run> {
    match cell {
...
fn held_needs_lift<'run>(cell: &Held<'run>, dying_frame: &Rc<FrameStorage>) -> bool {
    match cell {
