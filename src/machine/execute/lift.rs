...
#[cfg(test)]
pub fn lift_ktype_for_test<'a>(t: &KType<'a>, dying_frame: &Rc<CallArena>) -> KType<'a> {
    lift_ktype(t, dying_frame)
}

pub(super) fn lift_kobject<'a>(v: &KObject<'a>, dying_frame: &Rc<CallArena>) -> KObject<'a> {
...
pub(super) fn lift_ktype<'a>(t: &KType<'a>, dying_frame: &Rc<CallArena>) -> KType<'a> {
    match t {
        KType::Module {
            module: m,
            frame: existing,
...
fn any_descendant<'a, F>(v: &KObject<'a>, predicate: &F) -> bool
where
    F: Fn(&KObject<'a>) -> Option<bool>,
{
...
fn needs_lift<'a>(v: &KObject<'a>, dying_frame: &Rc<CallArena>) -> bool {
    let dying_runtime: *const RuntimeArena = dying_frame.arena();
    any_descendant(v, &|obj: &KObject<'a>| match obj {
...
fn lift_held<'a>(cell: &Held<'a>, dying_frame: &Rc<CallArena>) -> Held<'a> {
    match cell {
...
fn held_needs_lift<'a>(cell: &Held<'a>, dying_frame: &Rc<CallArena>) -> bool {
    match cell {
...
fn kfuture_borrows_dying_arena<'a>(t: &KFuture<'a>, arena: &RuntimeArena) -> bool {
    if std::ptr::eq(
...
fn argvalue_borrows_arena<'a>(v: &ArgValue<'a>, arena: &RuntimeArena) -> bool {
    match v {
...
fn expression_borrows_arena<'a>(expr: &KExpression<'a>, arena: &RuntimeArena) -> bool {
    expr.parts
        .iter()
        .any(|p| part_borrows_arena(&p.value, arena))
}

fn part_borrows_arena<'a>(part: &ExpressionPart<'a>, arena: &RuntimeArena) -> bool {
    match part {
        ...
        ExpressionPart::Future(Carried::Object(obj)) => {
            arena.owns_object(*obj as *const KObject<'a>)
        }
...
/// KExpression arm.
fn kobject_borrows_arena<'a>(v: &KObject<'a>, arena: &RuntimeArena) -> bool {
    any_descendant(v, &|obj: &KObject<'a>| match obj {
