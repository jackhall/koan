...
/// reclaims unconditionally, so quiescence means an entirely free store.
#[test]
...
/// the next producer's delivery onto whatever took that index. Inv-C has the walk (or the splice)
/// drop a listed edge's name; this asserts nothing slipped past it.
#[test]
