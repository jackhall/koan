...
//!
//! When a bare-name slot resolves its name to a binding-producer, its result *is* that producer's
//! result. Rather than keep a forwarding node (the old `Lift`), the slot is **spliced out**: it
//! becomes an alias of the producer, which stays the single producer of that result. This module
//! owns every graph operation the splice needs, so the alias contract lives in one place:
//!
//! - [`Scheduler::splice_forward`] performs the splice — move the consumers already parked on the
//!   slot onto the producer's notify list, and mark the slot an alias (in [`NodeStore`]) so reads
//!   follow through.
...
//!   *resolved* producer. A consumer that wires to the slot *after* the splice (a pre-submitted
//!   eager sub whose owner parks late) therefore waits on — and is woken by — the real producer,
//!   not the dead alias. A resolved producer that has already finalized adds no edge at all: its
//!   value is read directly when the consumer runs, contributing nothing to its pending count.
...
impl<'run> Scheduler<'run> {
    ...
    pub(in crate::machine::execute::scheduler) fn splice_forward(
        &mut self,
        slot: NodeId,
        producer: NodeId,
    ) {
