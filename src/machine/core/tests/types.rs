...
fn register_type_inserts_into_types_map_not_data() {
    ...
    let _ = scope.register_type_direct("Foo".into(), KType::NUMBER, DeclarationSite::BUILTIN);
...
fn resolve_type_walks_outer_chain_and_returns_none_past_root() {
    ...
    let _ = root.register_type_direct("Foo".into(), KType::NUMBER, DeclarationSite::BUILTIN);
...
fn resolve_type_inner_scope_shadows_outer() {
    ...
    let _ = root.register_type_direct("Foo".into(), KType::NUMBER, mock_declaration_site(1, 1));
    let child = region.brand().alloc_scope(Scope::child_under(root));
    let _ = child.register_type_direct("Foo".into(), KType::STR, mock_declaration_site(2, 1));
...
fn child_module_reach_names_the_child_region_which_owns_its_members_reaches() {
    ...
    source_scope
        .bind_value_direct("m".to_string(), sealed, None, BindingIndex::value(0))
