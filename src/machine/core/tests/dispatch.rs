...
fn pending_overload_parks_only_on_exact_bucket_match() {
    ...
    match scope.resolve_dispatch(
        ...
        DispatchOutcome::ParkOnProducers(ps) => assert_eq!(&*ps, &[ProducerId::for_test(42)]),
        other => panic!(
...
fn finalized_pick_with_pending_sibling_parks_until_finalize() {
    ...
    let pick_num_fn =
        KFunction::alloc_captured_draft(scope, pick_num, Body::Builtin(body_a), &registries);
    scope
...
fn parked_bare_name_parks_before_any_pick() {
    ...
    match scope.resolve_dispatch(
        ...
        DispatchOutcome::ParkOnProducers(ps) => assert_eq!(&*ps, &[producer]),
        other => panic!(
