...
impl ScopeId {
    ...
    /// Content bytes for a type digest: `session` then `idx`, each little-endian. An
    /// `AbstractType`'s binder and its generativity nonce, and a generative set's nonce, fold
    /// their `ScopeId` into the digested type content through this — a `Signature`'s `sig_id` is
    /// diagnostic/specificity-only and never folded in.
