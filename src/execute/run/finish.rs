use crate::dispatch::{
    BodyResult, CombineFinish, Frame, KError, KFuture, KObject, NodeId, Parseable, Scope,
};
...
impl<'a> Scheduler<'a> {
    pub(in crate::execute) fn run_bind(
        &mut self,
    ...
    pub(in crate::execute) fn run_combine(
        &mut self,
        deps: Vec<NodeId>,
    ...
    /// by construction.
    pub(in crate::execute) fn run_lift(&self, from: NodeId) -> NodeOutput<'a> {
    ...
    pub(in crate::execute) fn invoke_to_step(
        &mut self,
        future: KFuture<'a>,
        scope: &'a Scope<'a>,
        idx: usize,
