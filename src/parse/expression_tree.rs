...
fn peel_part<'a>(brand: ProgramBrand<'a>, part: ExpressionPart<'a>) -> ExpressionPart<'a> {
    match part {
        ExpressionPart::Expression(inner) => {
            ExpressionPart::Expression(brand.alloc_node(peel_redundant(brand, *inner)))
        }
...
pub fn parse_with_source<'a>(
    ...
    root.parts
        ...
        .map(|part| match part.value {
            ExpressionPart::Expression(e) => Ok(peel_redundant(program, *e).node()),
