//! Slot-array tests: the three-state transitions, the live-claim counter, and the one-allocation
//! shape.

use super::{SlotArray, SlotConflict, SlotState};
use crate::memory::program_storage;

/// A stand-in payload and producer — both `Copy` and drop-free, the tier a bump-hosted cell admits.
type Cells<'a> = SlotArray<'a, u32, u8>;

#[test]
fn cells_start_empty_and_unclaimed() {
    let program = program_storage();
    let cells: Cells<'_> = SlotArray::new(program.brand().region().allocator(), 3);
...
fn a_commit_retires_its_own_claim() {
    let program = program_storage();
    let mut cells: Cells<'_> = SlotArray::new(program.brand().region().allocator(), 2);
...
fn an_unsatisfied_claim_retires_to_empty() {
    let program = program_storage();
    let mut cells: Cells<'_> = SlotArray::new(program.brand().region().allocator(), 1);
...
/// A cell is drop-free whatever it holds — the fact the bump-hosted array rests on, stated against
/// the cell type the way the array's own constructor asserts it.
#[test]
fn cells_carry_no_drop_glue() {
    assert!(!std::mem::needs_drop::<SlotState<u32, u8>>());
