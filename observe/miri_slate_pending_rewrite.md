# Miri audit slate (pending rewrite)

<!-- slate-fingerprint

-->

**The old runtime's slate.** Every test here pins `machine`, `builtins` or
`memory`'s value-driving suite, which compile only under the `pending_rewrite`
feature; run it with `python3 tools/miri.py --pending-rewrite`, which selects
this file and turns the feature on. It is frozen with the runtime it audits and
goes when that runtime does. The slate for the modules the rewrite keeps is
[miri_slate.md](miri_slate.md).

The canonical list of tests Miri's tree-borrows mode signs off on for the old
runtime's memory safety. Each test is a minimal-shape mirror of an unsafe site in the
...
- 2026-09-11: 1702s — 35 tests, 0 leaks, 0 UB
- 2026-09-10: 2119s — 35 tests, 0 leaks, 0 UB
