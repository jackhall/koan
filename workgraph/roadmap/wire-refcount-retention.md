...
edges, § Refcount reclamation, § Terminals and retention).

**Problem.** The dep graph runs two parallel accounting systems that disagree at
their seams. Edges carry an `Owned`/`Notify` kind consumed only by the free
cascade; retention pulls are seeded at `finalize` from the then-parked consumer
count, while ready-at-wire consumers are counted through a separate `owed`
channel (`DepGraph::owe_late_pull`); and discharge is split across three verbs
with different partial rules — `reclaim_deps` discharges `owed` but not edges,
`free` discharges both and force-drops holds, and the cascade force-kills owned
children regardless of other readers. `alloc_node`'s ready-dep paths skip the
accounting entirely (`alloc.rs:29–48`): a ready park dep is neither counted nor
...
wire after a hold's release silently no-ops in `owe_late_pull` while
`dep_delivered` `expect`s a live hold (`scheduler.rs:216`). The same wiring
semantics exist twice — `alloc_node`'s inline loops and the
`add_owned_edge`/`add_park_edge` facades behind `install_edges` — which is how
the paths skewed.
...
- Every wire adds one standing destination to its alias-resolved producer at
  installation; a consumer's end-of-step and its death release its wires through
  the same verb, exactly once (Inv-B in
...
- Top-level roots carry a run-held destination the embedder releases at the
...
- Owner-death semantics — decided. Refcount-natural: a producer survives its
  ...
  cascade is removed.
- Root pulls — decided. The run holds one destination per entry slot, released
  at the drain boundary; embedder-driven and id-based (no RAII guard, no global
  registry).
- Reclaim trigger — open. (a) Slots are born at count zero and reclaim only on
  ...
  (b) allocation confers a birth pull that wiring transfers to the consumer.
  Recommended: (a) — no pull-transfer bookkeeping, and the root pull is just an
  ordinary standing destination.
- Where the count lives — open. On the `DepRow` from slot birth (hold
