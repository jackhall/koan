...
//! scheduler stores it and hands it back but inspects it not at all. The slot's realized dep list is not part of the work: the install doors write it onto the
...
/// of the dep edges an install door wired onto the slot. The continuation is held opaquely
...
///
/// A node carries no diagnostic payload. What a stuck slot renders as at a deadlock is the
/// embedder's question, answered off the per-slot anchor the workload already owns
/// ([`Workload::Frame`]) — so no workload-shaped label rests in scheduler storage.
...
/// A node's work in its resting form: the continuation sealed on the owned tier against the slot's anchor `Rc`. The anchor transitively holds the storage chain the
