    ...
    Cells<'graph, C, S, W>
{
    ...
    fn take_halves(&mut self, cell: CellHandle) -> Halves<'graph, C, S> {
        match cell {
            CellHandle::Slab(handle) => {
                let cell = &mut self.slots[handle.slot() as usize];
    ...
    fn put_halves(&mut self, cell: CellHandle, halves: Halves<'graph, C, S>) {
        match cell {
            CellHandle::Slab(handle) => {
                let cell = &mut self.slots[handle.slot() as usize];
