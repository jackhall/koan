...
/// One cell's step slots: its continuation in two halves, its receipt run, and a registration
...
> {
    ...
    /// The scratch half of the continuation, at rest: what names the cell's scratch bump across a
    ...
    StepSlots<'graph, C, S, D>
{
    ...
    /// with no scratch half, no run and no registration — its first step is what puts any of them
    /// there.
    ...
    /// Whether anything at rest here names the scratch bump the cell writes: its scratch half, or
    ...
    pub(crate) fn names_scratch(&self) -> bool {
        self.scratch_continuation.is_some() || self.receipts.is_some()
    }
    ...
    /// Both halves of the continuation, off the cell for the length of its step. Each has its own
    /// family — the storage half re-anchors at the executing cell's `'here` and the scratch half
    /// at `'scratch` — so neither slot can hold the other's form.
    pub(crate) fn take_halves(&mut self) -> (Option<Erased<'graph, C>>, Option<Erased<'graph, S>>) {
        (self.continuation.take(), self.scratch_continuation.take())
    }

    /// Both halves back at rest, as the step left them.
    pub(crate) fn put_halves(
        &mut self,
        (continuation, scratch): (Option<Erased<'graph, C>>, Option<Erased<'graph, S>>),
    ) {
        self.continuation = continuation;
        self.scratch_continuation = scratch;
    }
    ...
    /// on a cell that will never read it. The storage half stays: it is not what holds the reset
    /// off, and the disposal is what takes it.
    pub(crate) fn clear_scratch(&mut self) {
        self.scratch_continuation = None;
        self.receipts = None;
