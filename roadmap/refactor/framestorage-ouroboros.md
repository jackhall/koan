...
Replace the hand-rolled arena↔child-scope self-reference in `FrameStorage` with an
...
in `Rc<FrameStorage>`: the per-call `Scope` is allocated *into* `self.arena` and its
`arena: &'a RuntimeArena` field points back at that same allocation. The borrow checker cannot
express a field borrowing from a sibling field, so the loop is closed by hand — `CallArena::new`
and `try_reset_for_tail` take `&storage.arena` as a raw pointer and `pin_deref` it to `&'static`,
...
(reached by `CallArena::scope` / `scope_for_bind`). Soundness rests on a prose invariant (the
`Rc` heap-pins the arena; the brand bounds the pointer) rather than a type the compiler checks.

**Acceptance criteria.**

- `FrameStorage`'s arena↔child-scope self-reference is an `ouroboros #[self_referencing]` struct;
  no `unsafe` is written to recover the child scope from the arena.
- `ScopePtr` (the unbounded-capable branded pointer) and `ScopePtr::reattach_unbounded` no longer
  exist; `CallArena` holds no `scope_ptr` field.
- The two `CallArena` arena `pin_deref` sites (`new`, `try_reset_for_tail`) are gone.
...
- *Full `ScopePtr` deletion (not just `CallArena`'s use) — decided.* `ScopePtr` also backs `Module`
...
- *Keep `CallArena` as the public shell over `Rc<FrameStorage>` — decided.* The `Rc` stays for
...
- *`try_reset_for_tail` rebuilds the struct rather than mutating the borrowed owner — decided.*
  ...
  generated `Drop` preserves the arena-before-outer order and escapee-holds-old-storage semantics.
- *Outer-link erasure and the escape redirect stay manual — decided.* Both are cross-frame
  (reached via the `outer: Rc` field / a sibling arena), not intra-struct, so ouroboros (which
  models only the arena self-ref) does not subsume them; `BoundedScopePtr` and `ScopeFamily`
...
Builds on shipped substrate — the arena unsafe-collapse work on branch
