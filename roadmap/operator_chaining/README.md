# Operator chaining

User-declarable operators and the n-ary chaining mechanism that evaluates them: a
recognized run of operators reduces by its group's declared mode — unary, fold, or
pairwise — and a module-scoped `GROUP`/`OP` surface populates the per-scope operator
registry the reducer walks.

## Next items

This project's items with no unshipped prerequisite — ready to start.
Regenerated by `python3 tools/doclinks.py sync-next`; do not edit by hand.

- [Reject the unquoted operator symbol](reject-the-unquoted-operator-symbol.md)
- [Unify the unary-operator registration shape](unify-unary-operator-registration.md)

