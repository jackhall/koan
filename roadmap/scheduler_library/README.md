...
- **The catch channel's watched-value delivery.** `CatchOk.value` /
  ...
  relocated copy — the last remaining `DepTerminal::relocate` caller. Carrier-only
  delivery for the catch channel follows the carrier-carrying pattern the dispatch
  finishes now use ([design/scheduler-library.md](../../design/scheduler-library.md)).
