...
entry when its fix lands; delete the section when empty. The correctness
findings and the `alloc_with`-consumer cleanup are tracked as their own items —
[fold-embedded-dep-reach](fold-embedded-dep-reach.md) and
[total-carrier-resolution](total-carrier-resolution.md) — not here.
...
4. **Three inline comments exceed the 3–4 line cap** (Claude.md:
   "Inline comments: keep these to 3-4 lines"):
   [attr.rs](../../src/builtins/attr.rs) ~330 (9 lines),
   [keyworded.rs](../../src/machine/execute/dispatch/keyworded.rs) ~327
   (7 lines), [ctx.rs](../../src/machine/execute/dispatch/ctx.rs) ~266
...
5. **A third scope-lookup ladder twin.**
   [scope.rs](../../src/machine/core/scope.rs) ~787's `resolve_type_carrier`
   copies `resolve_value_carrier`'s ancestor chain-walk, differing only in
   the (lookup, resident) verb pair and hit field — deepening the ladder
   [unify-scope-lookup-ladders](../refactor/unify-scope-lookup-ladders.md)
   targets. **Fix:** fold into that item's scope — one chain-walk
   parameterized over the channel pair.

6. **The scope-to-`FinishCtx` recipe exists in three places.**
...
- [Total carrier resolution for spliced names](total-carrier-resolution.md)
