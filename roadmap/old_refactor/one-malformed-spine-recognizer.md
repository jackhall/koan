...
(~170 raise sites); there is no shared recognizer, so a new builtin shape has to wire up its own
detector at every stage it wants to be diagnosed at. They group as:

- **Full-bucket-key probes.** [`builtin_shape_for`](../../src/parse/builtin_shapes.rs) and `BuiltinShape::matches`, the four
...
- One recognizer answers "which builtin shape is this spine a near-miss for, and how" and every
  stage above that diagnoses a near-miss reads it rather than deriving its own answer.
- Adding a builtin shape registers its diagnosable mistakes in one place; no stage carries a
...
- *Whether a diagnosable-mistake shape should be a `BuiltinShape` at all — open.* Four
  `BUILTIN_SHAPES` rows
...
- *Where the recognizer lives — open.* Candidates: a further fact on a `BUILTIN_SHAPES` entry
  beside its run, or a `BuiltinShapeId`-keyed table beside `CLOSE_RULES` and `MISS_DIAGNOSTICS`.
- *How far the consolidation reaches — open.* Candidates: the full-bucket-key group alone (the
  stages that already probe `BUILTIN_SHAPES`), or every group including the scope walks and the parse-time
...
Better done after
[round-trip the builtin shapes under arbitrary layout](round-trip-builtin-forms.md), whose law over
