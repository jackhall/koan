# Round-trip the builtin shapes under arbitrary layout
...
keywords [`BUILTIN_SHAPES`](../../src/parse/builtin_shapes.rs) spells. No generated run therefore
matches a `BUILTIN_SHAPES` key, and three things go untested under arbitrary layout as a result: the parse-time
...
only rewrites at a masked slot of a matched key; a shape's raw-capture kinds and binder plan are never
...
([builtin_shapes/tests/binder.rs](../../src/parse/builtin_shapes/tests/binder.rs)) renders each
`BUILTIN_SHAPES` entry with
...
- The generator draws keywords from `BUILTIN_SHAPES`'s own spellings as well as from the disjoint pool, so a
...
- The round-trip law holds over builtin shapes under every layout the tape spells — indented child
  lines, redundant wrappers, ignorable commas — and the type-slot flip is asserted inside it rather
  than by a separate pin.
- A node's cached entry, its binder plan and its raw-capture kinds are asserted on a run rendered
  at a tape-chosen layout, not only at the canonical spelling.

**Directions.**

- *Where the builtin keys enter the generator — open.* Candidates: a second `Tree` arm that draws a
  whole `BUILTIN_SHAPES` entry and fills its slots recursively, or widening the keyword pool and letting the
...
- *How the oracle learns the rewrite — open.* Candidates: the oracle reads `type_slots` off the same
  `BUILTIN_SHAPES` entry the generator drew (one source of truth, but the law then cannot catch a wrong mask),
