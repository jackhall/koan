...
- A module body's window still sits inline in `ScopeKind::Module` with no allocation and no `Rc` of
  ...
  ([memory-model.md](../../old_design/memory-model.md)) holds unchanged — frame death stays `O(1)`.
...
- *Where a declarator-local window is hosted — open.* A standalone declaration has no module scope
  ...
  ([step scratch is reset at every drain pop](../../old_design/per-node-memory.md)).
...
- [Seal as a value-bearing producer](seal-as-producer.md) — the seal terminal carries the one
  window type.
- [Definitions park on free names](../old_foundation/definitions-park-on-free-names.md) — the
