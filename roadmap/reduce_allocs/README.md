...
- [Builtin body allocations](builtin-body-allocations.md)
...
- **Bounded-but-data-dependent bump fills** — a `BumpVec` that outgrows its reservation
  ...
  [fn_def/signature.rs](../../src/builtins/fn_def/signature.rs), which
  [Builtin body allocations](builtin-body-allocations.md) leaves on the heap for exactly this
  reason: it grows by `extend(producers)` over a `TypeResolution::Park(Vec<ProducerId>)`
