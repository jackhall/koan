...
the binding tables go bump-backed (keys and entry payloads alike), and `Scope`
itself then follows every other family into the region bump as a structurally
`Drop`-free resident (deleting the typed-storage machinery, the `typed-arena`
dependency, and the last storage-side lifetime retype) — plus one policy
