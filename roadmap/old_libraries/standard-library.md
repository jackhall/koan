...
- The standard effect modules (`Random`, `IO`, `Time` from
  [monadic-side-effects](../old_foundation/monadic-side-effects.md)) ship as stdlib entries
...
- *`Result` combinator set — open.* `map`, `bind`, `unwrap_or`, and
  ...
  [old_design/error-handling.md](../../old_design/error-handling.md)), so user
...
- *Applicative functor semantics — deferred to predicate typing.* Stage
  ...
  [old_design/typing/functors.md](../../old_design/typing/functors.md), two
...
- [Monadic side effects](../old_foundation/monadic-side-effects.md) — the `Monad`
