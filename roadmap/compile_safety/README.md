...
| [Cross-region pin cycles](region-debug-audits.md) | over-pin | pin bundles holding `Rc<FrameStorage>` members with no cycle detection — the self and eternal rules cut the two known ring shapes by construction, but nothing detects a third (`reach.rs`) | `Region::retain_reach` and every mint door reaching it through `Region::retain_for` (`Scope::mint_retained`, `Delivered::adopt_into` / `open_adopted`, `Carrier::compose_into`, `Sectioned::build`) |
