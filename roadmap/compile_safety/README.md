...
whose composition derives the reach — and the one runtime check left, the
per-family `AuditedStored` reattach guard (an `unsafe impl` declaring a single
`ptr::eq` against the region the value's own field names), is itself slated for
deletion by [Delete the audited reattach tier](audited-tier-deletion.md). The
remaining work is that deletion plus the hotspot-map holes below — the
...
- [Delete the audited reattach tier](audited-tier-deletion.md)
