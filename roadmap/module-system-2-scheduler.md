...
input via `<Type: E.Type>` named-slot syntax. Two stage-2 unsafe sites
...
- *Higher-kinded type slots and sharing constraints become expressible.*
  ...
  input via `<Type: E.Type>` named-slot syntax. Unblocks the in-language
...
- [Eager type elaboration with placeholder-based recursion](eager-type-elaboration.md) —
  HKT slot elaboration (`KType::TypeConstructor`) and `<Type: E.Type>`
