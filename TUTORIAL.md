...
| `<verb:TypeExprRef> (<args>)`            | Construct a tagged or struct value, e.g. `Maybe (some 42)` or `Point (x: 3, y: 4)`.             | [scheduler/dispatch/single_poll.rs](src/machine/execute/scheduler/dispatch/single_poll.rs) (`ConstructorCall` fast lane) |
| `<verb:Identifier> (<args>)`             | Call a function, tagged-union type, or struct type bound under `<verb>`.                        | [scheduler/dispatch/fn_value.rs](src/machine/execute/scheduler/dispatch/fn_value.rs) (`FunctionValueCall` fast lane) |
| `<s>.<field>` (`ATTR <s> <field>`)       | Read `<field>` off a struct value. Compound-token `.` operator; `s.x.y` chains.                  | [attr.rs](src/builtins/attr.rs)                      |
| `<v:Identifier>` (single-part)           | Look up `<v>` in scope.                                                                         | [scheduler/dispatch/single_poll.rs](src/machine/execute/scheduler/dispatch/single_poll.rs) (`BareIdentifier` fast lane) |
