...
Subcommands:
  ...
  sync-next            regenerate roadmap/README.md's "Next items" list from the
                       roadmap dependency graph (items with no open prerequisite)
...
def render_next_block(readme: Path, next_paths: list[Path]) -> list[str]:
...
def cmd_sync_next(args: argparse.Namespace) -> int:
    """Regenerate roadmap/README.md's `## Next items` section from the graph."""
    ...
    new_block = render_next_block(readme, next_paths)
    section = find_section(lines, NEXT_ITEMS_HEADER)

    if section is None:
        # No section yet: insert before `## Open items` if present, else append.
        old_set: set[Path] = set()
        insert = find_section(lines, "open items")
        pos = insert[0] if insert else len(lines)
        new_lines = lines[:pos] + new_block + lines[pos:]
        changed = True
    else:
        start, end = section
    ...
    new_set = {p.resolve() for p in next_paths}
    ...
    print(f"{verb} {rel(readme)} ({len(next_paths)} next item(s))")
    return 0


def _check_next_items() -> int:
    """Gate: the `## Next items` section must equal what `sync-next` would emit."""
    readme = REPO / "roadmap" / "README.md"
    if not readme.exists():
        print("roadmap/README.md not found.", file=sys.stderr)
        return 1
    lines = readme.read_text(encoding="utf-8").splitlines(keepends=True)
    section = find_section(lines, NEXT_ITEMS_HEADER)
    next_paths = compute_next_items()
    expected = render_next_block(readme, next_paths)

    if section is None:
        print("no '## Next items' section "
              "(run: python3 tools/doclinks.py sync-next)")
        print("\nNext-items list is missing.", file=sys.stderr)
        return 1
    current = lines[section[0]:section[1]]
    if current == expected:
        print("Next items list matches the dependency graph.")
        return 0

    cur_set = _links_under_roadmap(current, readme.parent)
    exp_set = {p.resolve() for p in next_paths}
    for p in sorted(exp_set - cur_set):
        print(f"  missing from Next items: {rel(p)}")
    for p in sorted(cur_set - exp_set):
        print(f"  stale in Next items:     {rel(p)}")
    if cur_set == exp_set:
        print("  membership is correct but the section text is stale "
              "(title or formatting drift).")
    print("  fix: python3 tools/doclinks.py sync-next")
    print("\nNext-items list is out of sync with the dependency graph.",
          file=sys.stderr)
    return 1
...
def cmd_rm_roadmap(args: argparse.Namespace) -> int:
    ...
    # 2. roadmap/README.md: prune the shipped item's bullet from Open items.
    #    The Next items list is derived from the dependency graph, so it is
    #    regenerated wholesale after the delete (below) rather than pruned here —
    #    deleting an item can also *add* newly-unblocked dependents to it.
    ...
    if roadmap_idx.exists():
        ...
        section = find_section(idx_lines, "open items")
        if section:
            kept, removed = remove_matching_bullets(
                idx_lines, section[0] + 1, section[1], target, roadmap_idx.parent,
            )
            if removed:
                new_lines = idx_lines[:section[0] + 1] + kept + idx_lines[section[1]:]
                plan.append((roadmap_idx, new_lines, removed))
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_sync = sub.add_parser(
        "sync-next",
        help="regenerate roadmap/README.md's 'Next items' list from the "
