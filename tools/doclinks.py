...
Subcommands:
  ...
  sync-next            regenerate the global "Next items" list in roadmap/README.md
                       plus each project README's slice (including each
                       embedded crate's roadmap/README.md), from the roadmap
...
# embedded crate's. They form one dependency graph — an edge crosses trees
...
def roadmap_items() -> list[Path]:
    """Every markdown file across every roadmap tree, README indexes included."""
    out: list[Path] = []
...
def under_roadmap(path: Path) -> bool:
    """Whether `path` names a file in any roadmap tree."""
...
# each embedded crate's `roadmap/README.md`. Same derived-and-gated contract as
# the global list, scoped to one project's items.
PROJECT_NEXT_INTRO = [
...
def cmd_sync_next(args: argparse.Namespace) -> int:
    ...
    The global list covers koan's own `roadmap/` tree only — each embedded crate's
    items are indexed by that crate's own `roadmap/README.md`, which
    `project_readmes` returns."""
...
def cmd_rm_roadmap(args: argparse.Namespace) -> int:
    ...
    if not under_roadmap(target):
        print(f"error: {args.path} is not under any roadmap/ tree",
              file=sys.stderr)
        return 1
    if not target.exists():
...
def _changed_src_files(base: str) -> list[tuple[str, Path, Path | None]]:
    """Run `git diff --name-status -M <base> -- <every crate's src>` and return
    ...
    try:
        proc = subprocess.run(
            ["git", "-C", str(REPO), "diff", "--name-status", "-M",
             base, "--", "src", *(f"{c}/src" for c in EMBEDDED_CRATES)],
